...
/**
 * Selector de nivel + pago DIRECTO en Stripe (flujo pago-primero). No exige login
 * ni tener empresa: primero se paga y luego, en /destacar/completar, el cliente
 * asigna el pago a una empresa suya.
 */
export function PrepaidCheckout({
  initialTier = "REGIONAL",
}: {
  initialTier?: FeaturedTier;
}) {
  const [state, formAction, pending] = useActionState(
    startPrepaidCheckout,
    INITIAL,
  );
  const [tier, setTier] = useState<FeaturedTier>(initialTier);
  ...
  return (
    <form action={formAction}>
      ...
      <Button
        ...
        className="mt-5 w-full sm:w-auto"
      >
        {pending ? "Redirigiendo a la pasarela..." : "Pagar y destacar"}
        <ArrowRight className="size-4" />
      </Button>
      <p className="text-muted-foreground mt-3 text-xs">
        Primero pagas de forma segura con Stripe; justo despues eliges a que
        empresa aplicar el destacado (puedes crearla o reclamarla en ese momento).
      </p>
      {state.error && (
        <p className="text-destructive mt-2 text-sm">{state.error}</p>
      )}
